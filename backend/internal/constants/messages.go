...
const (
	...
	MsgWithdrawNotFound      = "撤回重签申请（ReportWithdraw）不存在"
	MsgWithdrawNotPublished  = "仅已发布（published）报告可申请撤回"
	MsgWithdrawWindowExpired = "报告发布已超过 24 小时，撤回申请窗口已关闭"
	MsgWithdrawDuplicate     = "该报告已存在待审批的撤回申请，同一报告只能有一份待审批申请"
	MsgWithdrawNotPending    = "撤回申请已处理或已过期，无法重复/并发审批"
	MsgWithdrawFrozen        = "撤回审批期间冻结下载与新版本生成，请等待审批结束"
