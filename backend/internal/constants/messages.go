...
const (
	...
	MsgWithdrawNotFound       = "撤回申请（ReportWithdrawRequest）不存在"
	MsgWithdrawNotPublished   = "报告撤回失败：仅已发布（published）报告可申请撤回"
	MsgWithdrawWindowExpired  = "报告撤回失败：已发布超过 24 小时，撤回窗口已关闭"
	MsgWithdrawDuplicate      = "报告撤回失败：同一报告已存在待审批申请"
	MsgWithdrawFrozen         = "报告撤回审批中：下载与新版本生成已冻结"
	MsgWithdrawAlreadyHandled = "撤回申请审批失败：申请已处理，请勿重复或并发审批"
	MsgWithdrawTimeout        = "撤回申请审批失败：申请已超过 24 小时审批时限"
